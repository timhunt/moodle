...
$string['legacyfilesessions'] = 'Use the file system for session information (legacy)';
...
$string['memcachesessionhosts'] = 'Memcached host to use for sessions';
$string['memcachesessionhosts_desc'] = 'The name of the memcachehost must be set here, if you want to use memcache sessions. For example memcached.example.com.';
$string['memcachesessions'] = 'Use memcached for session information (experimental)';
...
$string['sessionhandling_desc'] = 'Controls how Moodle stores information about current sessions. Only choices that your server can handle are show. The following choices exist: Database sessions are the most reliable option, and are recommended if your database supports them. Legacy file sessions will work on any sever, and are OK for small sites, but do not work well for large sites uses several clustered servers. Note that changing this setting now will log out all current users (including you). Memcached sessions are a new experimental option that may work better for very busy sites. If you are using MySQL and want to use database sessions, please make sure that \'max_allowed_packet\' in my.cnf (or my.ini) is at least 4M.';
