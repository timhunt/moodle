...
class qformat_blackboard_six extends qformat_default {
    ...
    function process_block($cur_block, &$block) {
        ...
        switch($cur_type) {
            case 'FORMATTED_TEXT_BLOCK':
                ...
                break;
            case 'FILE_BLOCK':
                //this part need a complete rework for Moodle 2.0 and 2.1
                //
    ...
    function process_choices($bb_choices, &$choices) {
        foreach ($bb_choices as $choice) {
            ...
            } else if (isset($choice['#']['response_label'])) {
                // Reset $cur_choice to null because process_block is expecting an object
                // for the second argument and not a string, which is what is was set as
                // originally - CT 8/7/06
                $cur_choice = null;
    ...
    function process_feedback($feedbackset, &$feedbacks) {
        foreach ($feedbackset as $bb_feedback) {
            ...
            } else if (isset($bb_feedback['#']['solution'][0]['#']['solutionmaterial'][0]['#']['flow_mat'][0])) {
    ...
    function process_tf($quest, &$questions) {
        ...
        if ($correct) {  // true is correct
            $question->answer = 1;
            $question->feedbacktrue = $this->process_text($fback->correct);
            $question->feedbackfalse = $this->process_text($fback->incorrect);
        } else {  // false is correct
            $question->answer = 0;
            $question->feedbacktrue = $this->process_text($fback->incorrect);
            $question->feedbackfalse = $this->process_text($fback->correct);
        }
    ...
    function process_fblank($quest, &$questions) {
                ...
                if (isset($response->ident[0]['varequal'][0]['#'])) {
                    ...
                    if ($response->feedback = 'correct') {
                        ...
                        if (isset($feedback['correct'])) {
                            $feedbacks[] = $this->process_text($feedback['correct']);
                        } else {
                            $feedbacks[] = array('text'=>'', 'format'=>FORMAT_HTML, 'files'=>array());
                        }
                    }
                }
        ...
        if (isset($feedback['incorrect'])) {
            $feedbacks[] = $this->process_text($feedback['incorrect']);
        } else {
            $feedbacks[] = array('text'=>'', 'format'=>FORMAT_HTML, 'files'=>array());
        }
    ...
    function process_mc($quest, &$questions) {
        ...
        foreach ($quest->RESPONSE_BLOCK->choices as $response) {
            $question->answer[$i] = $this->process_text($response->text);
            if ($correct == $response->ident) {
                ...
                if (!empty($feedback['correct'])) {
                    $question->feedback[$i] = $this->process_text($feedback['correct']);
                } else if (!empty($feedback[$i])) {
                    $question->feedback[$i] = $this->process_text($feedback[$i]);
                } else {
                    // failsafe feedback (should be '' instead?)
                    $question->feedback[$i] = $this->process_text('correct');
                }
            } else {
                $question->fraction[$i] = 0;
                if (!empty($feedback['incorrect'])) {
                    $question->feedback[$i] = $this->process_text($feedback['incorrect']);
                } else if (!empty($feedback[$i])) {
                    $question->feedback[$i] = $this->process_text($feedback[$i]);
                } else {
                    // failsafe feedback (should be '' instead?)
                    $question->feedback[$i] = $this->process_text('incorrect');
                }
            }
            $i++;
        }
    ...
    function process_ma($quest, &$questions) {
        ...
        foreach ($choiceset as $choice) {
            $question->answer[$i] = $this->process_text(trim($choice->text));
    ...
    function process_essay($quest, &$questions) {
        ...
        $question->graderinfo =  array('text'=>'', 'format'=>FORMAT_HTML, 'files'=>array());
        if (!empty($question)) {
            $questions[]=$question;
        }
    }
    ...
    function process_matching($quest, &$questions) {
        ...
        if (question_bank::is_qtype_installed('ddmatch')) {
            $question = $this->process_common($quest);
            $question = $this->add_blank_combined_feedback($question);
            $question->valid = true;
            ...
            foreach ($quest->RESPONSE_BLOCK->subquestions as $qid => $subq) {
                ...
                foreach ($subq->choices as $cid => $choice) {
                    if ($choice == $correct) {
                        $question->subquestions[] = $this->process_text($subq->text);
                        $question->subanswers[] = $this->process_text($quest->RIGHT_MATCH_BLOCK->matching_answerset[$cid]->text);
                    }
                }
            }
            ...
            print "Matching question types are not handled because the quiz question type 'Drag-and-Drop Matching' does not exist in this installation of Moodle<br/>";
