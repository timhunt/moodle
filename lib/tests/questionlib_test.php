...
class questionlib_test extends \advanced_testcase {
    ...
    public function test_altering_tag_instance_context() {
        ...
        $questioncat = $questiongenerator->create_question_category(array('contextid' => $modcontext->id));
        ...
        \core_tag_tag::set_item_tags('core_question', 'question', $question1->id, $modcontext, array('tag 1', 'tag 2'));
        \core_tag_tag::set_item_tags('core_question', 'question', $question2->id, $modcontext, array('tag 1', 'tag 2'));
        ...
        $restoredcategory = $DB->get_record_select('question_categories', 'contextid = ? AND parent <> 0',
                array($qbank->context->id, '*', MUST_EXIST));
     ...
     */
    public function test_question_move_question_tags_to_new_context_activity_to_activity_qtags() {
        list($category, $course, $quiz, $qcat, $questions) = $this->setup_quiz_and_questions();
        $qbank = self::getDataGenerator()->create_module('qbank', ['course' => $course->id]);
        $question1 = $questions[0];
        $question2 = $questions[1];
    ...
    public function test_question_has_capability_on_using_moved_question($capabilities, $capability, $isowner, $expect) {
        ...
        $roleid = $generator->create_role();
        $course = $generator->create_course();
        $qbank1 = $generator->create_module('qbank', ['course' => $course->id]);
        $context = \context_module::instance($qbank1->cmid);
        $questioncat = $questiongenerator->create_question_category([
            'contextid' => $context->id,
        ]);

        $qbank2 = $generator->create_module('qbank', ['course' => $course->id]);
