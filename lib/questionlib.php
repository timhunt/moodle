...
function question_has_capability_on($questionorid, $cap, $notused = -1) {
    ...
    if (is_object($questionorid) && is_set($questionorid->contextid) && is_set($questionorid->createdby)) {
        $question = $questionorid;
    } else {
        if (is_object($questionorid)) {
            // Not sure if this next debugging line is backwards compatible, but it would be good to have.
            debugging('Object passed to question_has_capability_on should include both contextid and createdby fields', DEBUG_DEVELOPER);
            $questionorid = $questionorid->id);
        }
        $question = question_bank::load_question_data($questionorid)
    }
    ...
    if (!isset($capabilitieswithallandmine($capabilitieswithallandmine[$cap]) {
