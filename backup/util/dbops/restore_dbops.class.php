...
abstract class restore_dbops {
     ...
     * to target these contexts will now be attached to a 'fallback' mod_qbank
    ...
    public static function prechek_precheck_qbanks_by_level($restoreid, $courseid, $userid, $samesite, $contextlevel) {
        ...
        //         3b) User cannot. Move ALL the qcats to a default qbank module instance, warn. End qcat loop
        ...
        //             5b) User cannot. Move ALL the qcats to a default qbank module instance, warn. End qcat loop
            ...
            foreach ($categories as $category) {
                ...
                if (!$matchcat) {
                    ...
                    if ($canmanagecategory && $canadd) {
                        ...
                        $module = core_question\sharing\helper::get_default_open_instance_system_type($course, true);
                    ...
                    foreach ($questions as $question) {
                        ...
                        if (!$matchqid) {
                            ...
                            if ($canadd) {
                                ...
                                break 2; // out from qcat loop (both 5a and 5b), we have decided about ALL categories in context (bank)
                            }
        ...
        switch ($contextlevel) {
            ...
            case CONTEXT_MODULE:
                ...
                $sql = 'SELECT c.*, m.name AS modulename
                        FROM {context} AS c
                        JOIN {course_modules} AS cm ON c.instanceid = cm.id
                ...
                if (!empty($stamps) && !empty($supportedcontexts)) {
                    [$stamp_sql, $stamp_params] = $DB->get_in_or_equal($stamps);
                    $supportedcontexts = array_map(static fn($context) => $context->id, $supportedcontexts);
                    [$context_sql, $context_params] = $DB->get_in_or_equal($supportedcontexts);
